...
- **Alta de cuentas**: **no hay auto-registro** — el login no ofrece “Registrate”. Las cuentas las crea un usuario privilegiado desde la pantalla **Usuarios** (`POST /users` autenticado, ADMIN/CREATOR). Se crea siempre como `USER`; para un rol mayor, un CREATOR hace el alta y luego un `PATCH /users/:id/role`.
