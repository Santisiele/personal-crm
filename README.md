...
- **Casos de uso**: `CreateUser` (**solo privilegiado**; rechaza `name` duplicado con `UserNameTakenError` → 409; `name` es único **entre activos** vía índice parcial, así un nombre dado de baja se reutiliza), `ChangePassword`, `ChangeUserRole`, `ListUsers`, `ViewUser`, `DeactivateUser` (baja lógica, solo privilegiado).
...
- **Endpoints** (`UsersController`):
  - `POST /users` — **crear cuenta; solo privilegiados** (ADMIN/CREATOR, vía `UserAccessPolicy.canCreateUser`). **No hay auto-registro público**: exige Bearer token (`401` sin token, `403` si no es privilegiado). Crea **siempre un `USER`**; el `CreateUserDto` **no acepta `role`** (con `forbidNonWhitelisted`, mandar `role` da `400`). La elevación pasa solo por `PATCH /users/:id/role`. `409` si el nombre ya existe — el adaptador Prisma traduce la violación de unicidad (`P2002`) a `UserNameTakenError` en vez de dejar salir un 500 (cubre el caso de un nombre ocupado por un usuario borrado lógicamente).
  - `PATCH /users/me/password` — cambiar la propia contraseña (userId sale del `Actor`).
  - `PATCH /users/:id/role` — cambiar el rol; **autorizado por jerarquía** (`UserAccessPolicy.canAssignRole`). El **CREATOR** asigna cualquier rol. Un **ADMIN** gestiona el equipo común: mueve un target libremente entre `USER` y `ADMIN` (promueve un usuario, degrada un admin), pero **ambos extremos deben quedar en `{USER, ADMIN}`** — no puede otorgar `CREATOR` (`newRole` = CREATOR) ni tocar/degradar a un `CREATOR` (`currentRole` = CREATOR), que son las dos escaladas posibles. Cualquier otro caso (incluido un `USER` sobre sí mismo) → `403`. `404` si el usuario no existe. El controller pasa el `Actor` al caso de uso, que carga el target (su rol actual es parte de la decisión) antes de chequear la política.
  - `GET /users` — listar usuarios; **solo privilegiado** (ADMIN/CREATOR), si no `403`. Devuelve `UserView[]`.
  - `GET /users/assignable` — directorio mínimo (`{ id, name }`) para elegir a quién asignar/reasignar una tarea; **cualquier autenticado** (a diferencia de `GET /users`, que es privilegiado). No expone rol ni hash. Caso de uso `ListAssignableUsers`. La ruta se declara **antes** de `GET /users/:id` para que no la capture el `:id`.
...
- **Endpoints** (`TasksController`):
  - `POST /tasks` — crear tarea (`CreateTaskDto`: `title`, `description`, `assigneeId?`, `dueDate?`, `companyId?`, `contactId?`). Omitir `assigneeId` → para uno mismo; `null` → sin asignar (solo privilegiados); un id → a ese usuario (solo privilegiados, salvo que sea uno mismo). `dueDate` se valida como fecha ISO; `companyId`/`contactId` (empresa y un contacto de esa empresa) se guardan tal cual (no se validan contra los contextos `companies`/`contacts` — la UI ofrece solo contactos de la empresa elegida). La respuesta incluye `title`, `description`, `dueDate`, `companyId`, `contactId` y `status`.
  - `PATCH /tasks/:id` — editar contenido (`EditTaskDto`: `title?`, `description?`, `dueDate?`, `companyId?`, `contactId?`; en cada campo, omitir = dejar igual, `null` = limpiar, valor = setear). Así una tarea existente se vincula/desvincula de empresa y contacto. Autorizado por `canEdit` (dueño, asignado o privilegiado).
  ...
  - `PATCH /tasks/:id/assignee` — reasignar (`ReassignTaskDto`). Autorizado por `TaskAccessPolicy.canReassign`: el **dueño** siempre puede; además, un actor puede reasignar una tarea cuyo **asignado actual tiene un rol estrictamente inferior** al suyo (USER < ADMIN < CREATOR) — puede sacarle trabajo a alguien más junior, pero no a un par ni a un superior. El rol del asignado lo resuelve `ReassignTask` vía el `UserRepository`; el helper `outranks` (en `user-role.ts`) captura la jerarquía.
...
- **Gestión de usuarios y permisos.** Directorio, asignación de roles espejando la jerarquía de la API (el CREATOR asigna cualquiera; el ADMIN mueve entre USER↔ADMIN pero no ofrece CREATOR ni permite tocar a un CREATOR), alta y baja lógica. Más dashboard con métricas (vencidas/hoy/en progreso/completadas) y cambio de la propia contraseña.
...
6. **Auth JWT** (implementado): el seam `@CurrentActor()` ahora lee `request.user` que setea el `JwtAuthGuard` global; se entra con `POST /auth/login` → par access/refresh. El **access token expira** (`exp` 1h) y el **refresh token** (7d) se canjea en `POST /auth/refresh` por uno fresco; ambos son JWT stateless con un claim `purpose` que impide usar uno como el otro. La **verificación de tokens vive en el puerto `TokenIssuer`** (no en el guard), así el guard depende de una abstracción y los escenarios BDD pueden inyectar un TTL cero para probar el rechazo de un token expirado. `GET /`, `POST /auth/login` y `POST /auth/refresh` son `@Public()`; el resto (incluido `POST /users`) exige token. **Login por `name`**, único **entre activos** (índice parcial `app_user_name_active_key WHERE deleted_at IS NULL`, así un nombre de un usuario dado de baja se reutiliza); `CreateUser` rechaza duplicados con `UserNameTakenError` (→409) antes de guardar, y el índice parcial es el backstop (el adaptador traduce el `P2002`). Se usó `jsonwebtoken` y **no** `@nestjs/jwt` (este arrastraba un `@nestjs/common` duplicado que rompía la DI de Nest).
...
- **DB sin migraciones versionadas**: el índice único de `name` es **parcial** (`app_user_name_active_key ... WHERE deleted_at IS NULL`), aplicado a mano sobre la dev DB — Prisma no expresa índices parciales, así que `@unique` se quitó del schema y se maneja por migración manual. Un nombre de un usuario dado de baja se puede reusar; dos activos con el mismo nombre chocan. Si en algún momento se adopta `prisma migrate`, formalizarlo.
- **Columnas de baja lógica (`deleted_at`/`deleted_by`)**: se agregaron al schema en `app_user`, `contact` y `company` (la `task` ya las tenía). Como no hay migraciones versionadas, **aplicarlas a mano sobre la dev DB** antes de correr e2e/integración, si no las queries de baja/listado fallan:
  ...
  -- `name` uniqueness is PARTIAL (only among non-deleted rows), so a deactivated
  -- user's name can be reused. Replaces the full @unique constraint.
  ALTER TABLE app_user DROP CONSTRAINT IF EXISTS app_user_name_key;
  CREATE UNIQUE INDEX app_user_name_active_key ON app_user (name) WHERE deleted_at IS NULL;
  ```
...
- **Alta de cuentas (cerrado)**: **no hay auto-registro público**. `POST /users` exige un **actor privilegiado** (ADMIN/CREATOR, `UserAccessPolicy.canCreateUser`): `401` sin token, `403` si no es privilegiado. Crea **siempre un `USER`** (el DTO no acepta `role`; mandar uno → `400`); la única vía de elevación es `PATCH /users/:id/role`, gateada por `canAssignRole`. El primer `CREATOR` se **bootstrapea desde el seed** (`prisma/seed.ts`, idempotente) con `BOOTSTRAP_CREATOR_NAME`/`BOOTSTRAP_CREATOR_PASSWORD` (defaults de dev `creator`/`change-me-please` — **cambiar el password en cualquier entorno real**). Un nombre duplicado da `409` limpio (`UserNameTakenError`), incluso si lo ocupa un usuario borrado lógicamente (el adaptador Prisma traduce el `P2002`). Cubierto por `user_managment.feature` (privilegiado crea USER; USER común denegado), `role_assignment.feature` (elevación gateada) y e2e (201/401/403/400/409).
